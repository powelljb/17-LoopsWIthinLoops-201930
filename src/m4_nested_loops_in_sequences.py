...
def multiply_numbers(sequence_of_lists):
    ...
    for k in range(i):
        ...
        z = len(new_list)
        for j in range(z):
            new_list[j] = new_list[j]*n
        returned_list = returned_list + new_list
        n=n+1
...
def run_test_sum_numbers():
    ...
    expected = 19
    answer = sum_numbers(([1,4,6], [5], [3]))
    print('Expected and actual are:', expected, answer)

    expected = 28
    answer = sum_numbers(([4], [5], [2,9,8]))
    print('Expected and actual are:', expected, answer)

    expected = 48
    answer = sum_numbers(([18,1], [5], [20,1,3]))
    print('Expected and actual are:', expected, answer)
...
def print_characters(sequence_of_strings):
    ...
    #  ** ASK QUESTIONS if you do not understand the TESTS (ABOVE).
    # -------------------------------------------------------------------------
    i = len(sequence_of_strings)
    new_list = ''
    for k in range(i):
        new_list = sequence_of_strings[k]
        z = len(new_list)
        for j in range(z):
